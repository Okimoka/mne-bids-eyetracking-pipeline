...
"""
def read_raw_iview(sample_fname: str):
    ...
    # TODO this is just added so the current pipeline doesn't crash
    raw_et._raw_extras = [{
        "dfs": {
            "fixations": pd.DataFrame(columns=[
                c if c != "onset" else "time"
                for c in [
                    "onset","duration","end_time","trial_type","eye",
                    "fix_avg_x","fix_avg_y","fix_avg_pupil_size",
                    "sacc_start_x","sacc_start_y","sacc_end_x","sacc_end_y",
    ...
    # add annotations to the Raw. They are identical to annotations from the User Events in _Events.txt
    t0 = times[0]
    ...
    #event_fname = sample_fname.replace("_Samples", "_Events", 1)
    event_fname = sample_fname.copy().update(suffix="Events")
    
    if not os.path.exists(event_fname):
        logger.info(**gen_log_kwargs(message=f"Warning: SMI _Events.txt not found ({event_fname}). Returning Raw with inline MSG annotations only."))
        return raw_et

    with open(event_fname, "r", encoding="utf-8", errors="ignore") as ef:
        ev_lines = ef.readlines()

    event_types = ["Saccade", "Fixation", "Blink"]
    required = ("Start", "End", "Duration")

    # find/store all 3 table headers + indices; if anything missing, keep old raw_et
    header_info = {}
    for ev_type in event_types:
        marker = f"Table Header for {ev_type}"
        marker_idx = next((i for i, ln in enumerate(ev_lines) if ln.startswith(marker)), None)
        if marker_idx is None or marker_idx + 1 >= len(ev_lines):
            logger.info(**gen_log_kwargs(message=f"Warning: Missing table header for {ev_type} in _Events file. Returning without _Events data"))
            return raw_et

        cols = ev_lines[marker_idx + 1].rstrip("\n").split("\t")
        if any(r not in cols for r in required):
            logger.info(**gen_log_kwargs(message=f"Warning: Table header for {ev_type} missing one of {required}. Returning without _Events data"))
            return raw_et

        header_info[ev_type] = {
            "cols": cols,
            "idx_start": cols.index("Start"),
            "idx_end": cols.index("End"),
            "idx_dur": cols.index("Duration"),
        }

    # parse events; append into existing ann_* lists
    for ln in ev_lines:
        line = ln.rstrip("\n")
        if not line:
            continue
        ...
        start_us = float(parts[header_info[matched]["idx_start"]]) #µs
        dur_us = float(parts[header_info[matched]["idx_dur"]]) #µs

        ann_on.append((start_us - t0) / 1e6)
        ann_du.append(dur_us / 1e6)
        ann_ds.append(matched)

        # extras: columns starting at "Duration" column; keys named by the table header
        extra_colnames = header_info[matched]["cols"][header_info[matched]["idx_dur"]:]
        extra_values = parts[header_info[matched]["idx_dur"]:]

        extras_dict = {colname: float(val) for colname, val in zip(extra_colnames, extra_values)}
    ...
    *,
    ...
    for suffix, ext in variants:
        ...
        if suffix == "et" and ext == ".asc":
            ...
            logger.error(**gen_log_kwargs(
                message=f"Also didn't find {candidate} file, last try, .txt with suffix _Samples"
            ))
